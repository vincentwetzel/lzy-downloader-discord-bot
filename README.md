...
- **Startup Recovery:** If coordinator-owned downloads were saved in `downloads_backup.json`, the bot registers them before attaching to or starting LzyDownloader, prunes completed entries, and resumes progress tracking without losing startup webhook events. A request/expanded-child ID mismatch is recovered by normalized URL and download type.
...
- **Sleep/Wake Recovery:** The bridge watches prolonged Discord Gateway loss and exits after a bounded recovery period so the Windows supervisor can restart it; `on_ready` and `on_resumed` clear the watchdog after a successful reconnect. The launcher also prevents duplicate supervisor loops. `/stop` and the stop script write the shutdown marker before exiting so an intentional stop is not restarted.
...
- **Coordinator-Safe Cleanup:** The bridge does not terminate LzyDownloader when it exits because the coordinator may own the visible GUI or other automation work. Recovery state remains available for the next bridge session.
