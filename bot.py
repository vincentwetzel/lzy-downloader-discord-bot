...
# Load from .env, with a fallback to the default development path
LZY_EXECUTABLE_PATH = os.getenv('LZY_EXECUTABLE_PATH', r"E:\coding_workspaces\CPP\LzyDownloader\build-debug\Debug\LzyDownloader.exe")
...
def enforce_single_instance():
    ...
    try:
        ...
        print("❌ Another instance of the LzyDownloader Discord Bridge is already running. Exiting.")
        sys.exit(1)
...
class LzyBot(discord.Client):
    ...
    async def on_message(self, message: discord.Message):
        ...
        if is_valid_url(content):
            sent_msg = await message.channel.send(f"⏳ **Starting download:** <{content}>\n*Running in background...*")
            
            async def edit_msg(new_content):
                await sent_msg.edit(content=new_content)
                
            async def send_msg(new_content):
...
async def download(interaction: discord.Interaction, url: str):
    ...
    if not is_valid_url(url):
        await interaction.response.send_message("❌ Invalid URL provided. Please provide a valid HTTP/HTTPS link.", ephemeral=True)
        return
...
async def audio(interaction: discord.Interaction, url: str):
    if str(interaction.user.id) != AUTHORIZED_USER_ID:
        await interaction.response.send_message("❌ Unauthorized.", ephemeral=True)
        return
    ...
    await interaction.response.send_message(f"⏳ **Starting audio download:** <{url}>\n*Running in background...*")
    sent_msg = await interaction.original_response()
...
def check_api_health():
    ...
    try:
        ...
        if api_key:
            ...
            res = requests.get(f"{BASE_URL}/status", headers=headers, timeout=2, proxies=proxies)
    ...
    if not os.path.exists(LZY_EXECUTABLE_PATH):
        raise RuntimeError(
            f"**LzyDownloader Not Found.**\nExecutable not found at the configured path:\n`{LZY_EXECUTABLE_PATH}`\n\n"
            "Please update `LZY_EXECUTABLE_PATH` in `bot.py`."
        )

    # Clean up any stale token from a previous run so we don't try to use it
    key_path = os.path.join(os.getenv('LOCALAPPDATA'), 'LzyDownloader', 'Server', 'api_token.txt')
    ...
    process = subprocess.Popen([LZY_EXECUTABLE_PATH, "--server", "--exit-after"], cwd=app_dir, creationflags=getattr(subprocess, 'CREATE_NO_WINDOW', 0))
    ...
    for _ in range(20): # Poll for up to 20 seconds
        ...
        if process.poll() is not None:
            raise RuntimeError(f"**LzyDownloader crashed or closed immediately.** Exit code: {process.returncode}.\nTry running the .exe manually in a command prompt to check for missing DLLs.")
...
async def run_download_job(url: str, edit_callback, send_callback, download_type: str = "video"):
    ...
    try:
        enqueue_res = await asyncio.to_thread(requests.post, f"{BASE_URL}/enqueue", headers=headers, json={"url": url, "type": download_type}, timeout=5, proxies={"http": None, "https": None})
    ...
    while not completed:
        ...
        try:
            status_res = await asyncio.to_thread(requests.get, f"{BASE_URL}/status", headers=headers, timeout=5, proxies={"http": None, "https": None})
...
if __name__ == '__main__':
    ...
    if not TOKEN:
        ...
        print("ERROR: AUTHORIZED_USER_ID not found in .env file! Exiting to prevent unauthorized access to your machine.")
        sys.exit(1)
    else:
